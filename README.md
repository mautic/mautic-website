...
- The Knowledgebase - on [PHPKB](https://kb.mautic.org/)
- The Community Handbook - on [Read the Docs](https://contribute.mautic.org/)
- The MautiCon website - on [WordPress](https://mauticon.mautic.org/)
...
If at any point you’re not going to be able to complete the task, please call that out by sending the Team Lead a message to inform them, so that somebody else can pick it up. 

This transition can be achieved seamlessly because, unless there is a significant reason not to, we default to being open and transparent. We work in the open, usually on shared Mautic Google Drive or on GitHub pull requests. That way, if - for whatever reason - someone is unable to complete a task, it’s easy for another contributor to pick up where they left off. It also means we can always find previous work that has been done if it needs to be re-used in the future.

We totally understand that life happens, and it’s easy to take on too much. No judgment at all. We try to be respectful of each other by ensuring we give as much notice as possible if we’re not going to be able to fulfil a task assigned to us.
